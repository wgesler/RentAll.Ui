...
type VacantTurnoverRow = PropertyVacancyDisplay & {
  ...
  expandClick: (event: Event, item: VacantTurnoverRow) => void;
};

@Component({
  standalone: true,
  selector: 'app-dashboard-vacant',
  templateUrl: './dashboard-vacant.component.html',
  styleUrl: './dashboard-vacant.component.scss',
...
export class DashboardVacantComponent implements OnInit, OnDestroy {
  ...
  turnoverDisplayRows: VacantTurnoverRow[] = [];
  ...
  get checklistColumns(): ColumnSet {
    return {
      ...
      ...this.snapshot.vacantPropertyColumns
    };
  }

  get maintenanceColumns(): ColumnSet {
    const cols = this.snapshot.vacantMaintenanceColumns || {};
    return {
      ...cols,
  ...
  rebuildTurnoverDisplayRows(): void {
    ...
    for (const row of this.snapshot.vacantMaintenanceDisplay || []) {
    ...
    this.turnoverDisplayRows = (this.snapshot.vacantPropertyRows || []).map(row => {
      const propertyKey = this.normalizeKey(row.propertyId);
      ...
      const focusOnTab = focus?.tabIndex === 5 ? focus : null;
      ...
      return {
        ...
        expandClick: (event: Event, item: VacantTurnoverRow) => {
    ...
    const focus = this.companyDataService.calendarFocus?.tabIndex === 5 ? this.companyDataService.calendarFocus : null;
    this.orphanMaintenanceRows = (this.snapshot.vacantMaintenanceDisplay || [])
