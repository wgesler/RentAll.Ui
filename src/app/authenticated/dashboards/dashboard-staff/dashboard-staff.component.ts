...
import { BehaviorSubject, Subscription, filter, finalize, take, takeUntil } from 'rxjs';
...
export class DashboardStaffComponent extends PropertyMaintenanceBase implements OnInit, OnDestroy {
...
markViewForCheck(): void {
  ...
  protected override onAfterRecomputeBackendData(userAssignedId: string | null): void {
    ...
    const toEventDisplayRow = (row: ReservationPropertyMaintenance, eventType: ServiceType): ReservationPropertyMaintenance =>
      this.mixedMappingService.mapReservationPropertyMaintenanceDashboardStaffScheduleRow({
        ...row,
        eventType,
    ...
    for (const row of this.onlineProperties) {
      const eventRow = this.mixedMappingService.mapReservationPropertyMaintenanceDashboardStaffScheduleRow(
        this.mixedMappingService.mapPropertyMaintenanceToDashboardStaffScheduleRow(row)
      );
