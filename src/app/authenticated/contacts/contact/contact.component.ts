...
export class ContactComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes['preselectPropertyCodes'] || changes['preselectPropertyOfficeId']) {
      ...
      this.filterPropertiesByGlobalOffice();
      this.patchPreselectedPropertyCodesForAdd();
    }
  }
  ...
  saveContact(): void {
    ...
    if (entityTypeId === EntityType.Property) {
      const trimmedEmail = String(contactRequest.email || '').trim();
      if (!trimmedEmail) {
        const existingEmail = String(this.contact?.email || '').trim();
        if (this.utilityService.isAutoGeneratedContactEmail(existingEmail)) {
          contactRequest.email = existingEmail;
        } else {
          const fallbackOfficeId = Number.isFinite(resolvedOfficeId) && resolvedOfficeId > 0 ? resolvedOfficeId : 0;
          contactRequest.email = `property-contact-${fallbackOfficeId}-${Date.now()}@rentall.local`;
        }
      }
    }
  ...
  populateForm(): void {
    if (this.contact && this.form) {
      ...
      const insuranceExpirationDate = this.utilityService.parseDateOnlyStringToDate(this.contact.insuranceExpiration ?? null);
  ...
  applyEntityTypeContactValidators(entityTypeId: number | null | undefined): void {
    ...
    if (isCompany) {
      ...
      this.applyVendorNameCompanyValidators();
    } else if (entityTypeId === EntityType.Property) {
  ...
  applyPrefillContact(): void {
    ...
    const patch: Record<string, unknown> = {
      ...
      preferredName: String(this.prefillContact['preferredName'] ?? '').trim(),
  ...
  getPreselectedPropertyCodes(): string[] {
    const fromInput = Array.isArray(this.preselectPropertyCodes) ? this.preselectPropertyCodes : [];
    if (fromInput.length > 0) {
      return fromInput;
    }
    return this.dialogData?.preselectPropertyCodes ?? [];
  }

  getPreselectedPropertyOfficeId(): number | null {
    const inputOfficeId = Number(this.preselectPropertyOfficeId);
    if (Number.isFinite(inputOfficeId) && inputOfficeId > 0) {
      return inputOfficeId;
    }
    const dialogOfficeId = Number(this.dialogData?.preselectPropertyOfficeId);
    return Number.isFinite(dialogOfficeId) && dialogOfficeId > 0 ? dialogOfficeId : null;
  }

  mergePreselectPropertiesFromContext(): void {
    const codes = this.getPreselectedPropertyCodes()
      .map(c => String(c ?? '').trim())
      .filter(c => c.length > 0);
    const officeId = this.getPreselectedPropertyOfficeId();
  ...
  patchPreselectedPropertyCodesForAdd(): void {
    if (!this.form || !this.isAddMode) {
      return;
    }
    const entityTypeId = Number(this.form.get('entityTypeId')?.value);
    if (entityTypeId !== EntityType.Owner && entityTypeId !== EntityType.Property) {
      return;
    }
    const codes = this.getPreselectedPropertyCodes()
  ...
  initializeContactTypes(): void {
    ...
    const presetType = Number(this.presetEntityTypeId);
    const hasPreset = Number.isFinite(presetType) && this.availableContactTypes.some(option => option.value === presetType);
    if (Number.isFinite(presetType) && !hasPreset) {
      const label = getEntityType(presetType);
      if (label) {
        this.availableContactTypes = [{ value: presetType, label }, ...this.availableContactTypes];
      }
    }
  }
  ...
  applyPresetOfficeScope(): void {
    if (!this.form) {
      return;
    }

    const isPropertyPreset = Number(this.presetEntityTypeId) === EntityType.Property;
    const presetOfficeId = Number(this.presetOfficeId);
    if (!isPropertyPreset || !Number.isFinite(presetOfficeId) || presetOfficeId <= 0) {
      return;
    }

    this.form.patchValue(
      {
        officeAccess: [presetOfficeId],
  ...
  applyContactTypeLockedState(): void {
    ...
    const locked =
      ...
      (this.presetEntityTypeId === EntityType.Owner ||
        ...
        this.presetEntityTypeId === EntityType.Property);
  ...
  buildContactRequestWithUserId(savedContact: ContactResponse, originalRequest: ContactRequest, userId: string): ContactRequest {
    ...
    return {
      ...
      preferredName: savedContact.preferredName ?? originalRequest.preferredName,
