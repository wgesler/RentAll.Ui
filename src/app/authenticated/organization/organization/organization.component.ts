...
export class OrganizationComponent implements OnInit {
  ...
  formatAfterHoursPhone(): void {
    const afterHoursPhoneControl = this.form.get('afterHoursPhone');
    if (afterHoursPhoneControl && afterHoursPhoneControl.value) {
      const phone = afterHoursPhoneControl.value.replace(/\D/g, '');
      if (phone.length === 10) {
        const formatted = `(${phone.substring(0, 3)}) ${phone.substring(3, 6)}-${phone.substring(6)}`;
        afterHoursPhoneControl.setValue(formatted, { emitEvent: false });
      }
    }
  }
  ...
  onAfterHoursPhoneInput(event: Event): void {
    ...
    if (phone.length <= 10) {
      ...
      this.form.get('afterHoursPhone').setValue(formatted, { emitEvent: false });
    }
  }
