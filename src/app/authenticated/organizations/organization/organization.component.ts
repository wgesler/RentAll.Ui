...
export class OrganizationComponent implements OnInit, OnDestroy {
  ...
  saveOrganization(): void {
    ...
    const organizationRequest: OrganizationRequest = {
      ...
      suffixKeyName: (formValue.suffixKeyName || '').trim() || undefined,
    ...
    save$.pipe(take(1),finalize(() => this.isSubmitting = false)).subscribe({
      ...
      error: (err: HttpErrorResponse) => {
        console.error('Organization Component - Error saving organization:', err);
      }
    });
  }
  //#endregion
  ...
  populateForm(): void {
    if (this.organization && this.form) {
      this.form.patchValue({
        ...
        suffixKeyName: this.organization.suffixKeyName || '',
