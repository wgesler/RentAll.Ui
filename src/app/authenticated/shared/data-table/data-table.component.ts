...
export class DataTableComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy {
  ...
  @Input() linkActionTooltip: string = 'Copy Owner Link';
  @Input() linkActionColor: string = '#FF9800';
  @Input() hasActionsUser: boolean = false;
  @Input() hasActionsRental: boolean = false;
  @Input() hasActionsOwner: boolean = false;
  @Input() hasActionsPartner: boolean = false;
  ...
  @Input() hasActionsFix: boolean = false;
  ...
  @Output() partnerEvent = new EventEmitter<PurposefulAny>();
  ...
  @Output() fixEvent = new EventEmitter<PurposefulAny>();
...
markViewForCheck(): void {
  ...
  emitPartnerEvent(event: Event, rowItem: PurposefulAny): void {
    event.stopPropagation();
    this.partnerEvent.emit(rowItem);
  }
  ...
  emitFixEvent(event: Event, rowItem: PurposefulAny): void {
    event.stopPropagation();
    this.fixEvent.emit(rowItem);
  }
...
normalizeFilterValue(value: unknown): string {
  ...
  setActions(): void {
    ...
    if (this.hasActionsLink)     this.buttons.push({name: 'link', callback: (event, rowItem) => this.emitLinkEvent(event, rowItem), color: this.linkActionColor, tooltip: this.linkActionTooltip, tooltipPosition: 'before', icon: 'link', suspendOnUpdate: false});
    ...
    if (this.hasActionsFix)      this.buttons.push({name: 'fix', callback: (event, rowItem) => this.emitFixEvent(event, rowItem), color: 'primary', tooltip: 'Fix', tooltipPosition: 'before', icon: 'build', suspendOnUpdate: true});
